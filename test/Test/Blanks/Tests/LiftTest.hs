...
import Test.Blanks.Lib.LiftScope (LiftOuterBinder, LiftScope, appLift, baseLift, boundLift, closureLift,
                                  innerLamBinderLift, letLift, outerLamBinderLift, simpleLift, varLift)
...
data LiftCase = LiftCase
  ...
  , liftCaseBinders :: !(Map BinderId LiftOuterBinder)
...
liftCases =
  ...
  in [ LiftCase "var" svar (mkTrackedFree 'x') xvar Map.empty
    ...
    , let ib = innerLamBinderLift 'y' (letLift 'z' (varLift 'y') xbase)
          ob = outerLamBinderLift 0 [] ib
      in LiftCase "lamLetArgOuter" slamLetArgOuter mempty (closureLift 0 []) (Map.singleton 0 ob)
    , let ib = innerLamBinderLift 'y' (letLift 'z' (varLift 'x') xbase)
          ob = outerLamBinderLift 0 ['x'] ib
      in LiftCase "lamLetArgFree" slamLetArgFree mempty (closureLift 0 []) (Map.singleton 0 ob)
    , let ib = innerLamBinderLift 'y' (varLift 'x')
          ob = outerLamBinderLift 0 ['x'] ib
      in LiftCase "free" sfree mempty (closureLift 0 []) (Map.singleton 0 ob)
    , let ibY = innerLamBinderLift 'y' (varLift 'x')
          obY = outerLamBinderLift 0 ['x'] ibY
          ibZ = innerLamBinderLift 'z' (closureLift 1 [])
          obZ = outerLamBinderLift 0 ['x'] ibZ
      in LiftCase "free2" sfree2 mempty (closureLift 0 []) (Map.fromList [(0, obZ), (1, obY)])
    , let ib = innerLamBinderLift 'x' (varLift 'x')
          ob = outerLamBinderLift 0 [] ib
      in LiftCase "id" sid mempty (closureLift 0 []) (Map.singleton 0 ob)
    , let ib = innerLamBinderLift 'x' (boundLift 0)
          ob = outerLamBinderLift 1 [] ib
      in LiftCase "wonky" swonky (mkTrackedBound 0) (closureLift 0 [0]) (Map.singleton 0 ob)
    , let ibY = innerLamBinderLift 'y' (boundLift 0)
          obY = outerLamBinderLift 1 [] ibY
          ibZ = innerLamBinderLift 'x' (closureLift 1 [0])
          obZ = outerLamBinderLift 0 [] ibZ
      in LiftCase "const" sconst mempty (closureLift 0 []) (Map.fromList [(0, obZ), (1, obY)])
    , let ibY = innerLamBinderLift 'y' (varLift 'y')
          obY = outerLamBinderLift 0 [] ibY
          ibZ = innerLamBinderLift 'x' (closureLift 1 [])
          obZ = outerLamBinderLift 0 [] ibZ
      in LiftCase "flip" sflip mempty (closureLift 0 []) (Map.fromList [(0, obZ), (1, obY)])
    , let ib = innerLamBinderLift 'x' (boundLift 3)
          ob = outerLamBinderLift 1 [] ib
