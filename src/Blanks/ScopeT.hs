...
-- type instance BlankEmbedded (ScopeT t n f) = ScopeT t n f
...
-- instance (ApplicativeRightAdjunction t, Functor f) => BlankAbstract (ScopeT t n f) where
...
-- instance (ApplicativeRightAdjunction t, Functor f) => BlankBind (ScopeT t n f) where
--   blankBindN = scopeTBindN
--   blankBindOptN = scopeTBindOptN
...
-- scopeTBindOptN :: (ApplicativeRightAdjunction t, Functor f) => (a -> Maybe (RightAdjunct t (ScopeT t n f a))) -> Int -> ScopeT t n f a -> ScopeT t n f a
...
-- scopeTBindOpt :: (ApplicativeRightAdjunction t, Functor f) => (a -> Maybe (RightAdjunct t (ScopeT t n f a))) -> ScopeT t n f a -> ScopeT t n f a
...
-- subScopeTAbstract :: (ApplicativeRightAdjunction t, Functor f, Eq a) => Int -> n -> Seq a -> ScopeT t n f a -> RightAdjunct t (ScopeT t n f a)
-- subScopeTAbstract r n ks e =
--   let f = fmap scopeTBound . flip Seq.elemIndexL ks
--       e' = scopeTBindOpt f e
--   in scopeTBinder r n e'

-- scopeTAbstract :: (ApplicativeRightAdjunction t, Functor f, Eq a) => n -> Seq a -> ScopeT t n f a -> RightAdjunct t (ScopeT t n f a)
-- scopeTAbstract n ks =
--   let r = Seq.length ks
--   in subScopeTAbstract r n ks . scopeTShift r

-- scopeTUnAbstract :: (ApplicativeRightAdjunction t, Functor f) => Seq a -> ScopeT t n f a -> ScopeT t n f a
-- scopeTUnAbstract ks = scopeTInstantiate (fmap scopeTFree ks)

-- scopeTModOpt :: ApplicativeRightAdjunction t => (UnderScope n f (ScopeT t n f a) a -> Maybe (RightAdjunct t (ScopeT t n f a))) -> ScopeT t n f a -> ScopeT t n f a
-- scopeTModOpt f s = rightAdjunct (fromMaybe (pure s) . f) (unScopeT s)

-- scopeTModM :: (ApplicativeRightAdjunction t, Traversable m) => (UnderScope n f (ScopeT t n f a) a -> m (RightAdjunct t x)) -> ScopeT t n f a -> m x
...
-- scopeTInstantiateN :: (ApplicativeRightAdjunction t, Functor f) => Int -> Seq (RightAdjunct t (ScopeT t n f a)) -> ScopeT t n f a -> ScopeT t n f a
...
-- scopeTInstantiate :: (ApplicativeRightAdjunction t,  Functor f) => Seq (RightAdjunct t (ScopeT t n f a)) -> ScopeT t n f a -> ScopeT t n f a
-- scopeTInstantiate = scopeTInstantiateN 0

-- scopeTApply :: (ApplicativeRightAdjunction t, Functor f) => Seq (RightAdjunct t (ScopeT t n f a)) -> ScopeT t n f a -> Either SubError (ScopeT t n f a)
