...
data LiftFunctor f a
  = LiftFunctorBase !(f a)
  | -- | Bound variables - see 'liftAbstractClosureArity'.
    LiftFunctorAbstract !AbstractId !(Seq Int)
  deriving stock (Eq, Show, Generic, Functor, Foldable, Traversable)
  deriving anyclass (NFData)

-- | A lifted abstraction.
data LiftAbstract l c d f a = LiftAbstract
  { liftAbstractClosureArity :: !Int
  -- ^ Number of bound variables from enclosing scope. 'LiftFunctorAbstract' binds them.
  , liftAbstractFree :: !(Set a)
  -- ^ Tracked free variables in the abstraction
  , liftAbstractScope :: !(Abstract d (LocScope l c (LiftFunctor f) a))
  -- ^ Abstraction
  }
...
liftLocScopeInner
  ...
  -> State (LiftState l c d f a) (LiftResult l c f a)
...
predLiftLocScope
  ...
  => (forall x. n x -> Bool)
  -> LocScope (WithTracked a l) n f a
  -> State (LiftState l n n f a) (LiftResult l n f a)
